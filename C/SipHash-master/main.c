...
*/
int test(uint8_t *in,uint8_t * out, uint8_t * k){
  siphash(out,in,3,k);
  for (int i=0;i<8;i++){
  printf("%02x ", out[i] );
...
int main()
{
  uint8_t *k = (uint8_t *) malloc(16*sizeof(uint8_t)) ;
  k[0]=0x41;
  k[1]=0x42;
  k[2]=0x43;
  k[3]=0x44;
  ...
  k[14]=0x4f;
  k[15]=0x50;
  //uint8_t *k[16]={'a', 'b','c','d','e','f','g','h','i','j','k','l','m','n','o','p'};
  uint8_t *in= (uint8_t *) malloc(3*sizeof(uint8_t))  ;
  in[0]=0x00;
  in[1]=0x00;
  in[2]=0x00;
